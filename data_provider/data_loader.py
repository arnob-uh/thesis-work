...
class Dataset_telco(Dataset):
    def __init__(self, root_path, flag='train', size=None,
                 features='M', data_path='telco.csv',
                 target='None', scale=True, timeenc=0, freq='t'):
    ...
    def __read_data__(self):
        ...
        print(f"Number of columns in the dataset: {df_raw.shape[1]}")
    ...
    def __getitem__(self, index):
        ...
        seq_x = self.data_x[s_begin:s_end].astype(np.float32)  # Convert to float32
        seq_y = self.data_y[r_begin:r_end].astype(np.float32)  # Convert to float32
        seq_x_mark = self.data_stamp[s_begin:s_end].astype(np.float32)  # Convert to float32
        seq_y_mark = self.data_stamp[r_begin:r_end].astype(np.float32)  # Convert to float32
